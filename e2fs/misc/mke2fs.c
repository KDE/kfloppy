/* Hacked these sources so that they work with kfloppy
 *
...
const char * program_name = "kmke2fs";
...
static void test_disk(ext2_filsys fs, badblocks_list *bb_list)
{
	...
	bbfile = tmpfile();
	/*bbfile = fopen("/tmp/Bernd","w+");*/
	dev = open (fs->device_name, O_RDONLY);
	if (dev == -1)
	{
	...
	test_ro (dev, fs->super->s_blocks_count, 1024, bbfile, 0);
	rewind(bbfile);
	retval = ext2fs_read_bb_FILE(fs, bbfile, bb_list, invalid_block);
